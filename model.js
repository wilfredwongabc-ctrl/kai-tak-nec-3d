...
export function build(scene,renderer){
  ...
  function wall(a){
    ...
    const thickness=a.length>5?p(a[5]):(m==="tile"?.12:m==="painted"?.15:.26);
    const q=new T.Mesh(new T.BoxGeometry(n,3.15,thickness),M[m]||M.concrete);
  ...
  function frame(x,z,w,o){
    ...
    if(o==="h"){
      a=new T.Mesh(new T.BoxGeometry(.07,h,.1),M.metal);b=a.clone();
      a.position.set(p(x-w/2),h/2,p(z));b.position.set(p(x+w/2),h/2,p(z));
      H=new T.Mesh(new T.BoxGeometry(p(w),.07,.1),M.metal);
    }else{
      a=new T.Mesh(new T.BoxGeometry(.1,h,.07),M.metal);b=a.clone();
      a.position.set(p(x),h/2,p(z-w/2));b.position.set(p(x),h/2,p(z+w/2));
      H=new T.Mesh(new T.BoxGeometry(.1,.07,p(w)),M.metal);
    }
  ...
  for(let i=0;i<5;i++){
    const q=new T.Mesh(new T.BoxGeometry(.43,.58,.18),M.metal);
    q.position.set(p(120+i*13),1.08,p(58));
    L.shell.add(q);
  }
  const screen=new T.Mesh(new T.BoxGeometry(.82,.48,.07),new T.MeshStandardMaterial({color:0x111519}));
  screen.position.set(p(175),1.66,p(58));
  L.shell.add(screen);

  routes.forEach(a=>{
    let[x,z,X,Z,t]=a.map((v,i)=>i<4?p(v):v),dx=X-x,dz=Z-z,n=Math.hypot(dx,dz);
    const q=new T.Mesh(t==="duct"?new T.BoxGeometry(n,.30,.52):new T.BoxGeometry(n,.10,.10),t==="duct"?M.metal:M.pipe);
    q.position.set((x+X)/2,t==="duct"?2.78:3.0,(z+Z)/2);
    q.rotation.y=-Math.atan2(dz,dx);
    L.mep.add(q);
  });
  ...
  pts.forEach(v=>{const q=new T.Mesh(new T.SphereGeometry(.11,16,12),M.blue);q.position.copy(v);L.route.add(q);});
  return L;
}
