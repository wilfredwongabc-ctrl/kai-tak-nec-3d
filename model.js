...
import {S,W,D,walls,columns,zones,pits,routes,labels,doors,videoRoute} from "./data.js?v=12";
...
export function build(scene,renderer){
  ...
  const PLAN_W=1462/S,PLAN_D=475/S,PLAN_X=16/S,PLAN_Z=11/S;
  ...
  L.plan.add(plan);
  new T.TextureLoader().load("./assets/latest-plan.svg?v=12",tex=>{
    tex.colorSpace=T.SRGBColorSpace;
    tex.anisotropy=renderer.capabilities.getMaxAnisotropy();
    planMat.map=tex;planMat.needsUpdate=true;
  },undefined,e=>console.error("Unable to load exact floor-plan underlay",e));
  ...
  function wall(a){
    ...
    q.position.set((x+X)/2,1.575,(z+Z)/2);q.rotation.y=-Math.atan2(dz,dx);
    q.castShadow=q.receiveShadow=true;L.shell.add(q);
  }
  walls.forEach(wall);
  ...
  columns.forEach(a=>{
    ...
    q.position.set(p(a[1]),a[5]/2,p(a[2]));q.castShadow=q.receiveShadow=true;L.shell.add(q);
  });

  zones.forEach(a=>{
    const q=new T.Mesh(new T.PlaneGeometry(p(a[3]),p(a[4])),M[a[5]]);
    q.rotation.x=-Math.PI/2;q.position.set(p(a[1]),.018,p(a[2]));q.receiveShadow=true;L.shell.add(q);
  });
  ...
  function pit(poly){
    ...
    q.rotation.x=-Math.PI/2;q.position.y=.028;L.shell.add(q);
    const pts=poly.map(v=>new T.Vector3(p(v[0]),.05,p(v[1])));pts.push(pts[0].clone());
  ...
  for(let i=0;i<9;i++){const q=new T.Mesh(new T.CylinderGeometry(.05,.05,.85,12),new T.MeshStandardMaterial({color:0xeeeeea,roughness:.8}));q.position.set(p(535+i*11),.43,p(112+(i%3)*13));L.shell.add(q);}

  function frame(x,z,w,o){
    const h=2.25;let a,b,H;
    if(o==="h"){a=new T.Mesh(new T.BoxGeometry(.09,h,.12),M.metal);b=a.clone();a.position.set(p(x-w/2),h/2,p(z));b.position.set(p(x+w/2),h/2,p(z));H=new T.Mesh(new T.BoxGeometry(p(w),.09,.12),M.metal);}
    else{a=new T.Mesh(new T.BoxGeometry(.12,h,.09),M.metal);b=a.clone();a.position.set(p(x),h/2,p(z-w/2));b.position.set(p(x),h/2,p(z+w/2));H=new T.Mesh(new T.BoxGeometry(.12,.09,p(w)),M.metal);}
    H.position.set(p(x),h,p(z));L.door.add(a,b,H);
  }
  function leaf(x,z,w,o,s=1,m="blue"){
    const q=new T.Mesh(new T.BoxGeometry(p(w),2.15,.05),m==="public"?M.public:(M[m]||M.blue));
    q.position.set(p(x),1.075,p(z));q.rotation.y=(o==="h"?0:Math.PI/2)+s*.38;L.door.add(q);
  }
  doors.forEach(a=>{
    const col=a[5]==="painted"?"painted":a[5];frame(a[1],a[2],a[3],a[4]);
    if(a[0]==="double"){leaf(a[1]-a[3]/4,a[2],a[3]/2,a[4],-1,col);leaf(a[1]+a[3]/4,a[2],a[3]/2,a[4],1,col);}
    else leaf(a[1],a[2],a[3],a[4],a[5],a[6]);
  });

  for(let i=0;i<5;i++){const q=new T.Mesh(new T.BoxGeometry(.48,.62,.2),M.metal);q.position.set(p(120+i*13),1.12,p(58));L.shell.add(q);}
  const screen=new T.Mesh(new T.BoxGeometry(.9,.52,.08),new T.MeshStandardMaterial({color:0x111519}));screen.position.set(p(175),1.7,p(58));L.shell.add(screen);
  ...
  routes.forEach(a=>{
    ...
    q.position.set((x+X)/2,t==="duct"?2.75:2.98,(z+Z)/2);q.rotation.y=-Math.atan2(dz,dx);L.mep.add(q);
  });

  function rr(c,x,y,w,h,r){c.beginPath();c.moveTo(x+r,y);c.arcTo(x+w,y,x+w,y+h,r);c.arcTo(x+w,y+h,x,y+h,r);c.arcTo(x,y+h,x,y,r);c.arcTo(x,y,x+w,y,r);c.closePath();}
  labels.forEach(a=>{
    const cv=document.createElement("canvas"),c=cv.getContext("2d");cv.width=560;cv.height=90;c.font="bold 22px Arial, Microsoft JhengHei";
    const w=Math.min(530,c.measureText(a[0]).width+35);c.fillStyle="rgba(255,255,255,.94)";c.strokeStyle=a[3];c.lineWidth=4;rr(c,3,3,w,55,10);c.fill();c.stroke();c.fillStyle=a[3];c.fillText(a[0],18,39);
    const q=new T.Sprite(new T.SpriteMaterial({map:new T.CanvasTexture(cv),depthTest:false,transparent:true}));q.scale.set(5.7,1,1);q.position.set(p(a[1]),3.7,p(a[2]));L.label.add(q);
  });
